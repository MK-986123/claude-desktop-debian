...
# Conditional sandbox: only disable if root or unprivileged user namespaces are disabled
...
# Add Wayland-specific flags (for Electron 38+, ozone-platform=auto is default, but we set explicitly)
